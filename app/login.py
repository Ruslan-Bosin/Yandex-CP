...
class User:
    ...
    def create_with_user_id(self, user_id: str):
        ...
        if user_id.startswith("c"):
            ...
            self._user = session.query(ClientModel).filter(ClientModel.id == int(user_id[1::])).first()
            self._role = "client"
            ...
            self._user = session.query(OrganizationModel).filter(OrganizationModel.id == int(user_id[1::])).first()
