...
def client_login():
    ...
    if form.validate_on_submit():
        ...
        if email_validator(email) is None and password_validator(password) is None:
            ...
            client_account = db.session.query(ClientModel).filter(ClientModel.email == email).first()
...
def client_signup():
    ...
    if form.validate_on_submit():
        ...
        if name_validator(name) is None and email_validator(email) is None and \
            ...
            email_taken = db.session.query(ClientModel).filter(ClientModel.email == email).all()
            if email_taken:
...
def client() -> str:
    ...
    data = list()
    for item in sorted(db.session.query(RecordModel).filter(RecordModel.client == int(current_user._user.id)).order_by(RecordModel.accumulated).all(), key=lambda item: item.accumulated / db.session.query(OrganizationModel).filter(OrganizationModel.id == item.organization).first().limit, reverse=True):
        record_info = item.to_dict()
        organization_info = db.session.query(OrganizationModel).filter(OrganizationModel.id == record_info["organization"]).first().to_dict()
        client_info = db.session.query(ClientModel).filter(ClientModel.id == record_info["client"]).first().to_dict()
        data.append({
            "organization_id": organization_info["id"],
            "organization_title": organization_info["title"],
            "organization_sticker": organization_info["sticker"],
            "organization_limit": organization_info["limit"],
            "accumulated": record_info["accumulated"]
        })
...
def client_settings_change_name():
    ...
    if form.validate_on_submit():
        ...
        if name_validator(name) is None:
            ...
            client_account = db.session.query(ClientModel).filter(ClientModel.id == current_user._user.id).first()
...
def organization_login():
    ...
    if form.validate_on_submit():
        ...
        if email_validator(email) is None and password_validator(password) is None:
            ...
            organization_account = db.session.query(OrganizationModel).filter(OrganizationModel.email == email).first()
...
def organization_signup():
    ...
    if form.validate_on_submit():
        ...
        if name_validator(title) is None and email_validator(email) is None and password_validator(password) is None \
            ...
            email_taken = db.session.query(OrganizationModel).filter(OrganizationModel.email == email).all()
            if email_taken:
                flash("Данный email уже занят")
            else:
...
def organization():
    ...
    if form.validate_on_submit():
        ...
        client_exists = db.session.query(ClientModel).filter(ClientModel.id == int(id)).first()
        ...
        if client_exists:
            ...
            if record_exists:
                ...
                db.session.add(RecordModel(client=int(id), organization=int(current_user._user.id),
    ...
    data = list()
    for item in db.session.query(RecordModel).filter(RecordModel.organization == int(current_user._user.id)).order_by(RecordModel.accumulated.desc()).all():
        record_info = item.to_dict()
        organization_info = db.session.query(OrganizationModel).filter(OrganizationModel.id == record_info["organization"]).first().to_dict()
        client_info = db.session.query(ClientModel).filter(ClientModel.id == record_info["client"]).first().to_dict()
        data.append({
            "client_name": client_info["name"],
            "client_is_private": client_info["is_private"],
            "accumulated": record_info["accumulated"],
            "organization_limit": organization_info["limit"]
        })
    ...
    if request.method == "POST":
        ...
        if file and image_validator(file.filename) is None:
            try:
                ...
                organization_account = db.session.query(OrganizationModel).filter(OrganizationModel.id == current_user._user.id).first()
...
def organization_picture_get(id: int):
    ...
    for elem in db.session.query(OrganizationModel).order_by(OrganizationModel.id.desc()).limit(1):
        if elem.id > max_id:
            max_id = elem.id
    ...
    image = db.session.query(OrganizationModel).filter(OrganizationModel.id == id).first().image
