...
def rest_client_login():
    ...
    client = db.session.query(ClientModel).filter(ClientModel.email == email).all()[0]
    print(client)
    if not client:
        return make_response(jsonify({"message": "could not verify"}), 401)
...
def rest_client_signup():
    ...
    email_taken = db.session.query(ClientModel).filter(ClientModel.email == email).all()[0]
    if email_taken:
...
def rest_client_active(rest_user):
    ...
    data = db.session.query(RecordModel).filter(RecordModel.client == rest_user.id).order_by(RecordModel.accumulated.desc()).all()
...
def rest_client_change(rest_user):
    ...
    if name is not None:
        if name_validator(name) is None:
            ...
            client = db.session.query(ClientModel).filter(ClientModel.id == rest_user.id).all()[0]
            client.name = name
            # client.save()
            db.session.commit()
            changed.append("name")
...
def rest_organization_login():
    ...
    organization = db.session.query(OrganizationModel).filter(OrganizationModel.email == email).all()[0]
    if not organization:
        return make_response(jsonify({"message": "could not verify"}), 401)
...
def rest_organization_signup():
    ...
    email_taken = db.session.query(OrganizationModel).filter(OrganizationModel.email == email).all()[0]
    if email_taken:
        return make_response(jsonify({"message": "email taken"}), 417)
    ...
    if title_validator(title) is None and email_validator(email) is None and password_validator(password) is None:
        ...
        db.session.add(organization_account)
        db.session.commit()
...
def rest_organization_active(rest_user):
    ...
    data = db.session.query(RecordModel).filter(RecordModel.organization == rest_user.id).order_by(RecordModel.accumulated.desc()).all()
...
def rest_organization_change(rest_user):
    ...
    if title is not None:
        if title_validator(title) is None:
            ...
            organization = db.session.query(OrganizationModel).filter(OrganizationModel.id == rest_user.id).all()[0]
...
def rest_organization_record(rest_user):
    ...
    client_exists = db.session.query(ClientModel).filter(ClientModel.id == id).all()[0]

    if client_exists:
        # record_exists = RecordModel.get_or_none((RecordModel.client == id) & (RecordModel.organization == rest_user.id))
        record_exists = db.session.query(RecordModel).filter((RecordModel.client == id) &
                                                          (RecordModel.organization == rest_user.id)).all()[0]
        ...
        if record_exists:
            ...
            db.session.add(RecordModel(client=id, organization=rest_user.id, last_record_date=date.today()))
            db.session.commit()
...
def rest_admin_login():
    ...
    admin = db.session.query(AdminModel).filter(AdminModel.email == email).all()[0]
    if not admin:
        return make_response(jsonify({"message": "could not verify"}), 401)
...
def rest_admin_organizations(rest_user):
    ...
    data = db.session.query(OrganizationModel).all()
    result = list()
    for item in data:
        result.append({
            "id": item.id,
...
def rest_admin_create_record(rest_user):
    ...
    client_exists = db.session.query(ClientModel).filter(ClientModel.id == int(client_id)).all()[0]
    # organization_exists = OrganizationModel.get_or_none(OrganizationModel.id == int(organization_id))
    organization_exists = db.session.query(OrganizationModel).filter(OrganizationModel.id == int(organization_id)).all()[0]
    ...
    record_exists = db.session.query(RecordModel).filter((RecordModel.client == int(client_id)) &
                                                      (RecordModel.organization == int(organization_id))).all()[0]
    if record_exists:
    ...
    db.session.add(RecordModel(client=int(client_id), organization=int(organization_id), last_record_date=date.today()))
    db.session.commit()
...
def rest_admin_clients_update(rest_user):
    ...
    if name is not None:
        if name_validator(name) is None:
            ...
            client = db.session.query(ClientModel).filter(ClientModel.id == client_id).all()[0]
            client.name = name
            # client.save()
            db.session.commit()
            changed.append("name")
...
def rest_admin_clients_remove(rest_user):
    ...
    client_exists = db.session.query(ClientModel).filter(ClientModel.id == client_id).all()[0]
...
def rest_admin_organizations_update(rest_user):
    ...
    if title is not None:
        if title_validator(title) is None:
            ...
            organization = db.session.query(OrganizationModel).filter(OrganizationModel.id == organization_id).all()[0]
            organization.title = title
            # organization.save()
            db.session.commit()
            changed.append("title")
...
def rest_admin_organizations_remove(rest_user):
    ...
    organization_exists = db.session.query(OrganizationModel).filter(OrganizationModel.id == organization_id).all()[0]
...
def rest_admin_records_update(rest_user):
    ...
    if accumulated is not None:
        ...
        record = db.session.query(RecordModel).filter(RecordModel.id == record_id).all()[0]
...
def rest_admin_records_remove(rest_user):
    ...
    record_exists = db.session.query(RecordModel).filter(RecordModel.id == record_id).all()[0]
